...
async def query(db_session: DatabaseDependency) -> Page[CentroTreinamentoOut]:
    centros = await db_session.scalars(select(CentroTreinamentoModel))
    return paginate(centros.all())
...
async def get(id: UUID4, db_session: DatabaseDependency) -> CentroTreinamentoOut:
    centro_treinamento = await db_session.scalar(
        select(CentroTreinamentoModel).where(CentroTreinamentoModel.pk_id == id)
    )
